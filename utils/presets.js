...
const snapshotModsToPreset = db.transaction((presetId) => {
    ...
    const mods = db.prepare(`SELECT name, url, version, note, official, category, sort_order FROM mods ORDER BY sort_order, id`).all();
    const ins = db.prepare(`INSERT INTO preset_mods (preset_id, name, url, version, note, official, category, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
    for (const m of mods) ins.run(presetId, m.name, m.url, m.version, m.note, m.official, m.category, m.sort_order);
});

// Replace the live `mods` table with a preset's stored mod set.
const loadPresetIntoMods = db.transaction((presetId) => {
    const rows = db.prepare(`SELECT name, url, version, note, official, category, sort_order FROM preset_mods WHERE preset_id = ? ORDER BY sort_order, id`).all(presetId);
    db.prepare(`DELETE FROM mods`).run();
    const ins = db.prepare(`INSERT INTO mods (name, url, version, note, official, category, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)`);
    for (const m of rows) ins.run(m.name, m.url, m.version, m.note, m.official, m.category, m.sort_order);
});
...
function getPresetMods(presetId) {
    return db.prepare(
        `SELECT name, url, version, note, official, category, sort_order FROM preset_mods WHERE preset_id = ? ORDER BY sort_order, id`
    ).all(presetId);
}
